...
export function Wizard({ onBack, onFinish }: WizardProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f8fafc] text-[#0f172a] select-none font-sans">
      ...
      {/* ── Header ── */}
      <header className="flex items-center justify-between px-6 py-3 border-b border-slate-200/90 bg-white shadow-xs">
        <button
          id="wizard-back-btn"
          onClick={prev}
      ...
      <div className="h-0.5 bg-slate-100">
        <div
          className="h-full transition-all duration-500 ease-out bg-[#059669]"
          style={{ width: `${progress}%` }}
        />
      </div>

      {/* ── Content ── */}
      <div className="flex-1 flex items-start justify-center pt-10 pb-20 px-4">
        <div className="w-full max-w-lg">

          {/* Step indicator */}
          <div className="flex items-center gap-2 mb-5">
            <span className="text-[11px] font-bold text-[#059669] bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full font-mono">
            ...
            {q.options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={`relative text-left px-4 py-3.5 rounded-2xl border transition-all duration-150 ${
                    selected
